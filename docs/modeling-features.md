...
貫入双晶では、双晶軸まわりの回転に加えて、派生結晶を双晶軸方向へずらす `軸方向オフセット` を指定できます。オフセット量 `1` は、双晶軸と同じ指数で距離 `1` の面がその軸と交わる位置までの軸上距離を基準とし、値は線形に効きます。
...
文字が入っている面では、プレビュー上に文字輪郭の補助線も表示されます。位置、回転、サイズ、面内オフセットを確認したい時の目安として使えます。
...
補助的な出力機能として、結晶パラメーターカード内の `STL分割を有効にする（β版）` を使い、表示中の全結晶を 1 つの立体へまとめた後に指定平面で 2 分割した STL を保存できます。
